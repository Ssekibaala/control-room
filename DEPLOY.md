...
5. Under Environment, add every variable from `.env.example`:
...
6. Deploy. First deploy will be slow (installing dependencies), later
   ones are faster.

### 3. Apps Script scheduler
1. script.google.com -> New project.
2. Paste in `apps_script_scheduler.gs`.
3. Replace `RENDER_APP_URL` with your real Render URL + `/api/import`.
4. Replace `API_KEY` with the exact same string you set as
   `IMPORT_API_KEY` on Render.
...
Locally or via Render's Shell tab:
```
...
work:
```
curl -H "X-API-Key: your-import-api-key" https://your-app.onrender.com/api/import
```
Watch Render's Logs tab while this runs. If a report is missing or a
download link can't be found, the error will name exactly which report
subject failed, that's deliberate, see `mail_reader.py` and
`run_import.py`'s `fetch_reports()`.

### 6. Log in
Go to `https://your-app.onrender.com/`, sign in as one of the accounts
...
- **No SQLite, no Drive backup.** Feedback lives in Google Sheets
  ...
  cache lives only on Render's disk and is never backed up, because
  it's fully regenerable by re-running the import, nothing
...
- **Render's free tier sleeps.** The first request after idle time
  takes 30-60 seconds to wake up. The scheduled import can tolerate
  this; if a person is waiting on the dashboard cold, it'll just feel
  slow once, then it's warm.
...
Check Render's Logs tab first, the message there should tell you
